...
#
# API/routing service: serves HTTP only (players, raids, jobs, internal
# admin routes, card images). Enqueues simulation jobs but never runs them
# -- see Dockerfile.worker for the process that actually executes sims, kept
# on a separate deploy so a running simulation's CPU usage can never freeze
# this service's request handling.
